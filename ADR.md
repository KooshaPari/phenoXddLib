...
- The macros crate is a separate `proc-macro = true` crate per Rust requirements.
...
### Context
Quickcheck and proptest are the two dominant property testing libraries in the Rust ecosystem. Proptest has richer strategy composition and better shrinking.
...
### Context
FR traceability checks (orphan tests, uncovered FRs) should run at build time, not as a separate audit tool.
